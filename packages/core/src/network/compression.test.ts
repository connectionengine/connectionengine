...
describe('createBinaryPipeline — compression integration', () => {
  ...
  it('round-trips compressed Transform between two worlds within quantisation error', async () => {
    ...
    expect(got?.position.x).toBeCloseTo(5.5, 1)
    expect(got?.position.y).toBeCloseTo(-10.25, 1)
    expect(got?.position.z).toBeCloseTo(42, 1)
    expect(got?.rotation.z).toBeCloseTo(0.3827, 2)
    expect(got?.rotation.w).toBeCloseTo(0.9239, 2)
    destroyWorld(source)
    destroyWorld(target)
  })
})
